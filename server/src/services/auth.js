...
export const registerService = ({ phone, password, name }) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            err: token ? 0 : 2,
            msg: token ? 'register is' : ' phone number has user',
            token: token || null
        })
...
export const loginService = ({ phone, password, name }) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            err: token ? 0 : 2,
            msg: token ? 'login ok' : response ? 'password is wrong' : ' phone number no found',
            token: token || null
        })
